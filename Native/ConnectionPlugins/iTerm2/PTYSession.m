...
- (void)updateDisplay
{
    ...
    if ([[self tab] activeSession] == self) {
        ...
        if (!jobName_ ||
            ...
            if (isForegroundTab && [[self tab] parentWindow] && [[[self tab] parentWindow] tempTitle]) {
    ...
    if (anotherUpdateNeeded) {
        if ([[self tab] parentWindow] && [[[self tab] parentWindow] currentTab] == [self tab]) {
