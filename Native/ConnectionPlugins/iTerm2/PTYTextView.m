...
+ (void)initialize
{
    // FX EDIT - Image loading for framework
    
    NSPoint hotspot = NSMakePoint(4, 5);

    NSBundle* bundle = [NSBundle bundleForClass:[self class]];

    NSString* barCursorFile = [bundle
                          pathForResource:@"IBarCursor"
                          ofType:@"png"];
    NSImage* image = [[NSImage alloc] initWithContentsOfFile:barCursorFile];

    textViewCursor = [[NSCursor alloc] initWithImage:image hotSpot:hotspot];

    NSString* xmrCursorFile = [bundle
                               pathForResource:@"IBarCursorXMR"
                               ofType:@"png"];
    NSImage* xmrImage = [[NSImage alloc] initWithContentsOfFile:xmrCursorFile];
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
    ...
    // Clear buffer
    NSMenuItem *itemClearBuffer = [theMenu addItemWithTitle:NSLocalizedStringFromTableInBundle(@"Clear Buffer",@"iTerm", [NSBundle bundleForClass: [self class]], @"Context menu")
                                                     action:@selector(clearTextViewBuffer:)
                                              keyEquivalent:@"k"];
    
    [itemClearBuffer setKeyEquivalentModifierMask:NSCommandKeyMask];
    [[theMenu itemAtIndex:[theMenu numberOfItems] - 1] setTarget:self];
...
- (BOOL)_drawLine:(int)line AtY:(double)curY toPoint:(NSPoint*)toPoint
{
    ...
    BOOL useBackgroundIndicator = [[NSUserDefaults standardUserDefaults] boolForKey:@"Use background pattern indicator"];
    
    const BOOL stripes = useBackgroundIndicator &&
