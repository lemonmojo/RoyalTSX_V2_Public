...
- (id)initWithFrame:(NSRect)frame
{
    ...
    if (self) {
        [self _initCommon];
        findView_ = [[FindViewController alloc] initWithNibName:@"FindView" bundle:[NSBundle bundleForClass:[SessionView class]]];
...
- (void)doResize
{
    ...
    [self performSelectorOnMainThread:@selector(doResizeOnMainThread) withObject:nil waitUntilDone:YES];
}

- (void)doResizeOnMainThread
{
    @try {
        ...
        NSSize size = [self sessionSize];
        
        int width = size.width;
        int height = size.height;
        if (self.session.rows == height &&
            self.session.columns == width) {
            //PtyLog(@"PTYTab fitSessionToCurrentViewSize: noop");
            return;
        }
        if (width == self.session.columns && height == self.session.rows) {
            //PtyLog(@"fitSessionToWindow - terminating early because session size doesn't change");
            return;
        }
        
        [self.session setWidth:size.width height:size.height];
        
        [[self.session SCROLLVIEW] setLineScroll:[[self.session TEXTVIEW] lineHeight]];
        [[self.session SCROLLVIEW] setPageScroll:2*[[self.session TEXTVIEW] lineHeight]];
...
- (NSSize)sessionSize
{
    ...
    [[self.session SCROLLVIEW] setHasVerticalScroller:hasScrollbar];
    NSSize size = [[self.session view] maximumPossibleScrollViewContentSize];
    int width = (size.width - MARGIN*2) / [[self.session TEXTVIEW] charWidth];
    int height = (size.height - VMARGIN*2) / [[self.session TEXTVIEW] lineHeight];
    if (width <= 0) {
...
// FX EDIT, Added from PseudoTerminal
- (void)magnifyWithEvent:(NSEvent *)event
{
    ...
    cumulativeMag_ = 0;
    [[self session] changeFontSizeDirection:dir];
}
